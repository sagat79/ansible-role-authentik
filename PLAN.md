...
## Етап 3.5 — Brands (визуални настройки и defaults по домейн)

authentik Brands позволяват различен branding (заглавие, лого, favicon,
custom CSS) и различни default flows за всеки домейн, по който се достъпва
инстанцията (съвпадение по `Host` header).

1. **Преглед** на това, което ролята поддържа днес:
   - Traefik router-ът match-ва само `authentik_hostname` — заявки към
     допълнителни brand домейни изобщо не стигат до контейнера;
   - `authentik_cookie_domain` е един — да се документира поведението при
     няколко несвързани домейна;
   - логата/favicon файловете на brand-овете живеят в media storage
     (`/data`, вече поддържано, вкл. S3) — да се провери какви пътища очаква
     authentik при качване през blueprint vs. UI.
2. **Корекция**:
   - нова променлива `authentik_container_labels_traefik_additional_hostnames`
     (списък) — разширява router правилото до
     ``Host(`a`) || Host(`b`) …``, за да се сервират brand домейните;
   - декларативни brands: `authentik_brands` списък (domain, branding_title,
     branding_logo, branding_favicon, default flag), от който ролята генерира
     blueprint файл (`model: authentik_brands.brand`) чрез вече наличния
     blueprints механизъм — без кликане в UI;
   - валидация: точно един default brand; предупреждение при brand домейн,
     който липсва в Traefik hostnames списъка;
   - README раздел с пример за втори домейн.
3. **Тест**:
   - рендер тестове: router правило с няколко домейна; генерираният brands
     blueprint (валиден YAML, правилни identifiers/attrs); негативен тест за
     два default brand-а;
   - реален тест на тестовия сървър: втори домейн → различно лого/заглавие;
     основният домейн пада ли обратно към default brand-а; TLS сертификат за
     допълнителния домейн (certResolver покрива ли двата hostname-а).
...
4. Етапи 3.2–3.4 (blueprints, мониторинг, listen) и Етап 4 (документация). ✅
5. Етап 3.5 (Brands) — преглед, корекция и тест; реалният тест се комбинира с
   валидирането на ролята върху домашния тестов сървър.
6. Етап 5 (качество) — molecule сценарият остава отворен.
